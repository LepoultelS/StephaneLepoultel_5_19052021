...
formButton.addEventListener("click", function () {
  ...
  if (
    ...
    lastName.validity.valueMissing ||
    ...
    invalidMessage.textContent =
      "Vous devez accepter les conditions générales de ventes - CGV pour continuer.";
  } else if (
    specialCharactersValid(firstName.value) === false ||
    specialCharactersValid(lastName.value) === false ||
    specialCharactersValid(address.value) === false ||
    specialCharactersValid(city.value) === false
  ) {
    invalidMessage.textContent =
      "Les champs prénom/nom/adresse/ville ne doivent pas comporter de caractères spéciaux.";
  } else if (
    nameValid(firstName.value) === false ||
    nameValid(lastName.Value) === false
  ) {
    invalidMessage.textContent =
      "Les champs nom/prénom doivent être composés de lettres et ne doivent pas contenir de chiffres.";
  } else if (emailValid(email.value) === false) {
    invalidMessage.innerHTML = "Le champ email n'est pas correct";
  } else if (
    onlyWhitespacesInvalid(firstName.value) ||
    onlyWhitespacesInvalid(lastName.value) ||
    onlyWhitespacesInvalid(address.value) ||
    onlyWhitespacesInvalid(city.value) ||
    onlyWhitespacesInvalid(email.value)
  ) {
    invalidMessage.textContent =
      "Les champs ne doivent pas contenir uniquement des espaces.";
  } else {
    // if all form inputs are valid, create contact object & products array, set them to API and get response
    ...
    fetch("http://localhost:3000/api/teddies/order", requestOptions)
      ...
      .then((response) => {
        // get items of request reponse and put them in confirmation page URL, and redirect to this page
        document.location.href =
          "confirmation.html?firstName=" +
